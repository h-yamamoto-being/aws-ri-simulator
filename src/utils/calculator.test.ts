...
describe('calcMonthlyPaymentsJPY - オンデマンド', () => {
  it('毎月 hourlyRate × hoursPerMonth × rate の支払いになる', () => {
    const payments = calcMonthlyPaymentsJPY(onDemand(10, 100))
    // 10 × 100 × 150 = 150,000
  ...
  it('為替レートが月ごとに異なる場合は各月のレートを使う', () => {
    ...
    const scenario: HourlyScenario = { ...onDemand(10, 100), exchangeRates: rates }
    const payments = calcMonthlyPaymentsJPY(scenario)
...
describe('calcMonthlyPaymentsJPY - RI前払いなし', () => {
  it('毎月 onDemandMonthly × 0.72 × rate になる（28%割引）', () => {
    const payments = calcMonthlyPaymentsJPY(riNoUpfront(10, 100))
  ...
  it('オンデマンドより支払いが少ない', () => {
    const noUpfront = calcMonthlyPaymentsJPY(riNoUpfront())
    const demand = calcMonthlyPaymentsJPY(onDemand())
...
describe('calcMonthlyPaymentsJPY - RI一部前払い', () => {
  it('1ヶ月目は (前払い額 + 月額) × rates[0]', () => {
    const payments = calcMonthlyPaymentsJPY(riPartialUpfront(10000, 1000))
  ...
  it('為替レートが月ごとに異なる場合、2〜12ヶ月目はそれぞれのレートを使う', () => {
    const rates = Array.from({ length: 12 }, (_, i) => 100 + i * 10)
    const scenario: PartialUpfrontScenario = { ...riPartialUpfront(5000, 2000), exchangeRates: rates }
    const payments = calcMonthlyPaymentsJPY(scenario)
...
describe('calcMonthlyPaymentsJPY - RI全額前払い', () => {
  it('1ヶ月目のみ年間一括 (hourlyRate × hoursPerMonth × 12 × 0.68) × rates[0]', () => {
    const payments = calcMonthlyPaymentsJPY(riFullUpfront(10, 100))
  ...
  it('配列の長さは12', () => {
    expect(calcMonthlyPaymentsJPY(riFullUpfront())).toHaveLength(12)
  })
})
...
describe('calcScenarioResult', () => {
  ...
  it('全額前払いの totalJPY = 1ヶ月目の支払いのみ', () => {
    const result = calcScenarioResult(riFullUpfront(10, 100))
    expect(result.totalJPY).toBeCloseTo(result.monthlyPaymentsJPY[0])
  })

  it('RI前払いなしの年間合計がオンデマンドより安い', () => {
    const demandTotal = calcScenarioResult(onDemand()).totalJPY
    const noUpfrontTotal = calcScenarioResult(riNoUpfront()).totalJPY
